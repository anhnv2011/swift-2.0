...
class DataManager {
    ...
    class var sharedInstance: DataManager {
        ...
        dispatch_once(&Static.onceToken) {
            ...
            for item in raw!
            {
                let monAn = MonAn(name: item.value.valueForKey("name") as! String, photo: item.value.valueForKey("photo") as! String, chitiet: item.value.valueForKey("chitiet") as! String)
